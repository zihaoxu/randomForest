...
void rmultinomBLB(int n, int K, double* prob, int* rN)
  ...
  */
{
  ...
  //    if (K < 1) { ML_ERROR(ME_DOMAIN, "rmultinomBLB"); return;}
  //    if (n < 0)  ML_ERR_ret_NAN(0);
  //#else
  //    if (K == NA_INTEGER || K < 1) { ML_ERROR(ME_DOMAIN, "rmultinomBLB"); return;}
  ...
  */
  ...
  //        MATHLIB_ERROR(_("rbinomBLB: probability sum should be 1, but is %g"),
  //                      (double) p_tot);
  if (n == 0) return;
  if (K == 1 && p_tot == 0.) return;/* trivial border case: do as rbinomBLB */
